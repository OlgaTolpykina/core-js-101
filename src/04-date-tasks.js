...
function timeSpanToString(startDate, endDate) {
  ...
  const diffMilliseconds = endDate.getMilliseconds() - startDate.getMilliseconds();

  const format = (value, length) => value.toString().padStart(length, '0');

  return `${format(diffHours, 2)}:${format(diffMinutes, 2)}:${format(diffSeconds, 2)}.${format(diffMilliseconds, 3)}`;
}
