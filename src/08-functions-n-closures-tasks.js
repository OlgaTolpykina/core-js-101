...
function logger(func, logFunc) {
  return (...args) => {
    const strArguments = args.map((arg) => JSON.stringify(arg)).join(',');
    const startsResult = `${func.name}(${strArguments}) starts`;
    const endsResult = `${func.name}(${strArguments}) ends`;

    logFunc(startsResult);
    const resultFunction = func(...args);
    logFunc(endsResult);
